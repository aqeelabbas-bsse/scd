...
// Stronger mysqli error handling (prevents silent failures)
mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT);

// Helper: show alert and go back safely
...
// Password: 1 upper, 1 lower, 1 digit, 1 special, min 8
function is_strong_password($pass) {
    $pattern = "/^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[^A-Za-z0-9]).{8,}$/";
    return preg_match($pattern, $pass) === 1;
}

// Username (stored in full_name column): must contain letters + digits
// Allow spaces, but must have at least 1 letter and 1 digit
function is_valid_username_fullname($name) {
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($form_type === 'login') {
            ...
            // field name plugin ka: xoo_el_username (tum isko email OR username dono allow kar rahe ho)
            $login_input = trim($_POST['xoo_el_username'] ?? '');
            ...
            // ✅ If input is email → login by email
            // ✅ else login by full_name (username)
            if (is_valid_email($login_input)) {
                ...
                // username rule check (letters + digits)
                if (!is_valid_username_fullname($login_input)) {
                    js_back("Username must contain alphabets and numeric characters (letters + digits).");
                }
            ...
            if ($row = $result->fetch_assoc()) {
                ...
                if (password_verify($password, $row['password_hash'])) {
                    ...
                    js_back('Invalid username/email or password');
                }
        ...
        elseif ($form_type === 'register') {
            ...
            // NOTE: We will store username in full_name column (as you want)
            $email     = trim($_POST['xoo_el_reg_email'] ?? '');
            $pass1     = trim($_POST['xoo_el_reg_pass'] ?? '');
            $pass2     = trim($_POST['xoo_el_reg_pass_again'] ?? '');

            // Your form has fname/lname; we will combine them BUT enforce username rule on full_name
            $fname     = trim($_POST['xoo_el_reg_fname'] ?? '');
            $lname     = trim($_POST['xoo_el_reg_lname'] ?? '');
            $full_name = trim($fname . ' ' . $lname); // this is your "username" stored in full_name
            ...
            // ✅ Strong password
            if (!is_strong_password($pass1)) {
                js_back('Password must include uppercase, lowercase, number, special character, and be at least 8 characters long.');
            }

            // ✅ Username rule (full_name column): must contain alphabets + numeric
            // BUT your fname/lname normally alphabets only => digits won’t exist.
            // So user should include digits in fname or lname OR you can append digits in UI.
            if (!is_valid_username_fullname($full_name)) {
                js_back("Username (Full Name) must contain both alphabets and numeric characters. Example: Ali Malik 12");
            }
            ...
            // ✅ Duplicate username(full_name) check (recommended)
            ...
            if ($stmt->num_rows > 0) {
                $stmt->close();
                js_back('This username is already taken. Try adding different numbers.');
            }
            $stmt->close();
            ...
            js_back('Invalid form submission');
        }
    }

} catch (Throwable $e) {
    // Never crash the site
    js_back('Something went wrong. Please try again.');
}
...
/* ======END */
	...
	/* ===== FAQ SECTION (THEME-MATCH REDESIGN using SAME CLASSES) ===== */
/* ===== FAQ SECTION (MODERN + THEME MATCH) ===== */
.faq-section{
  width: 100%;
  max-width: 1100px;
  margin: 90px auto;
...
.faq-item::before{
  ...
  padding:1px;
  border-radius:18px;
  background: linear-gradient(135deg, rgba(34,165,180,0.45), rgba(34,165,180,0.08), rgba(0,0,0,0.05));
...
.faq-item:hover{
  ...
  border-color: rgba(34,165,180,0.35);
}
...
/* IMPORTANT: button reset so it never becomes a small left box */
.faq-question{
...
.faq-question span.icon{
  ...
  background: rgba(34,165,180,0.14) !important;
  color: #22a5b4 !important;
...
.faq-item.active .faq-question span.icon{
  ...
  background: rgba(34,165,180,0.22) !important;
}
...
@media (max-width: 768px){
  ...
  .faq-question{ font-size: 16px !important; padding: 18px 18px !important; }
...
<section class="faq-section" id="faqs">
  <div class="faq-title">
    ...
    <p>Quick answers to common questions about bookings, facilities, and your glamping experience.</p>
  </div>

  <div class="faq-item">
    <button type="button" class="faq-question">
      What is included in the glamping experience?
      <span class="icon">+</span>
    </button>
    <div class="faq-answer">
      Our glamping experience includes a fully furnished luxury tent or cabin, comfortable bedding, electricity, clean washroom facilities, and basic toiletries.
      Guests also get access to scenic outdoor areas, bonfire spots, and selected recreational activities depending on the location and package chosen.
    </div>
  </div>
  ...
  <div class="faq-item">
    ...
    <div class="faq-answer">
      Booking is simple and fully online. Choose your preferred glamp, select dates, and submit the booking form available on our website.
      Once your request is submitted, our team reviews the details and sends a confirmation along with further instructions via email or phone.
    </div>
  </div>
  ...
  <div class="faq-item">
    ...
    <div class="faq-answer">
      Cancellation policies depend on the selected package and season. Generally, cancellations made within a specific time frame may be eligible for a partial or full refund.
      We recommend contacting our support team with your booking reference to get accurate and up-to-date cancellation details.
    </div>
  </div>
  ...
  <div class="faq-item">
    ...
    <div class="faq-answer">
      Food availability varies by package. Some glamping packages include complimentary breakfast or full meals, while others offer meals as optional add-ons.
      Detailed meal information is provided during booking, and special dietary requirements can be requested in advance.
    </div>
  </div>

  <div class="faq-item">
    <button type="button" class="faq-question">
      Is the glamping site safe for families and children?
      <span class="icon">+</span>
    </button>
    <div class="faq-answer">
      Yes, our glamping sites are family-friendly and follow safety guidelines. The environment is secure, and staff are available on-site to assist guests.
      However, children should always be supervised, especially during outdoor activities.
    </div>
  </div>
